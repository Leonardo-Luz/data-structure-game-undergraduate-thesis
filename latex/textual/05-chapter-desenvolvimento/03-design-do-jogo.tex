...
A seguir, apresenta-se o personagem principal do jogo, responsável pelas misturas alquímicas e pela interação com o ambiente:
\input{figures/jogador}

O companheiro felino, que auxilia o jogador e integra o enredo, é mostrado a seguir:
\input{figures/gato}

A fogueira, que atua como ponto de controle e restauração do progresso, é ilustrada na imagem seguinte:
\input{figures/fogueira}

Os elementos alquímicos utilizados nas combinações que compõem as mecânicas centrais do jogo são apresentados abaixo:
\input{figures/elementos_alquimicos}

Da esquerda para a direita, os elementos são, respectivamente:
...
Os principais inimigos enfrentados durante a partida são mostrados na sequência:
\input{figures/inimigos}
...
A estrutura física do ambiente é composta por blocos modulares, responsáveis pela formação das plataformas e terrenos do cenário:
\input{figures/tiles}

As diferentes camadas de paralaxe, que compõem a profundidade visual do ambiente e reforçam a imersão, podem ser observadas, de forma sobreposta, na figura a seguir:
\input{figures/parallaxes}

Os consumíveis, que oferecem vantagens temporárias ou restauram atributos do jogador, são representados conforme a ilustração abaixo:
\input{figures/consumiveis}
...
Por fim, a \textbf{HUD} apresenta as principais informações de jogabilidade, como inventário, indicadores de vida e botões de interação. A imagem a seguir exibe a interface numerada, cuja descrição detalhada é apresentada logo abaixo:
\input{figures/hud}
...
\item Ponteiro da pilha;
\item Ponteiro da fila;
\item Ponteiro da lista;
