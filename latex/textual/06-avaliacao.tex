...
\input{tables/avaliacao-experiencia}
...
\input{graphs/01-engajamento}
\input{graphs/02-engajamento}
\input{graphs/03-engajamento}
...
\input{graphs/05-satisfacao}
\input{graphs/06-satisfacao}
...
\input{graphs/07-desafio}
\input{graphs/08-desafio}
...
\input{graphs/09-imersao}
\input{graphs/10-imersao}
...
\input{tables/avaliacao-usabilidade}
...
\input{graphs/11-estetica}
\input{graphs/12-estetica}
...
\input{graphs/13-aprendibilidade}
...
\input{graphs/14-operabilidade}
\input{graphs/15-operabilidade}
...
\input{graphs/16-acessibilidade}
\input{graphs/17-acessibilidade}
...
\subsection{Proteção Contra Erros}
...
\input{graphs/18-protecao-usuario}
...
\input{tables/avaliacao-conteudo}
...
\input{graphs/19-relevancia}
\input{graphs/20-relevancia}
\input{graphs/21-relevancia}
...
Em síntese, a análise qualitativa mostra que o jogo conseguiu atingir seu
propósito ao oferecer uma mecânica inovadora e engajadora para o ensino de
estruturas de dados. As críticas recebidas concentraram-se principalmente no
balanceamento da aleatoriedade e na necessidade de um \emph{onboarding} mais
gradual, pontos que foram registrados e incorporados como recomendações para
trabalhos futuros na \autoref{sec:trabalhos_futuros}.

A seguir, no \autoref{cap:consideracoes_finais}, são sintetizados os objetivos
alcançados e discutidos os próximos passos do projeto, considerando tanto os
avanços obtidos quanto as oportunidades de aprimoramento identificadas durante
a avaliação.
