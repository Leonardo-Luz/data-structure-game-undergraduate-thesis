...
diferenciar visualmente e logicamente os comportamentos LIFO (\textit{Last In,
First Out}) e FIFO (\textit{First In, First Out}), aplicar operações de inserção
(\textit{push/enqueue}) e remoção (\textit{pop/dequeue}) para resolver problemas
práticos, e compreender a necessidade de ordenação de dados para otimização de
buscas.
